        ...
        messages: [
          {
            ...
            content: {
              ...
              text: [
                ...
                "",
                "Steps:",
                `1. Make two analyze_feeds calls in parallel (each with months ${months}${args.folder ? `, folder "${args.folder}"` : ""}, limit 500) to surface both kinds of pruning candidates without truncation hiding either:`,
                `   - sort_by "days_since_newest" to surface DORMANT feeds first (they cluster at the top).`,
                `   - sort_by "unengaged_per_month" to surface high-volume NEVER-ENGAGED feeds first.`,
                "2. Compare the per-status counts in the summary to the feeds visible in each response. If the summary shows substantially more dormant or never-engaged feeds than appear in either result, mention this so the user knows the recommendations are not exhaustive.",
                "3. Combine the two responses (deduplicate by stream id) and group findings:",
                "   - DORMANT feeds (no content in 90+ days) -- suggest unsubscribe",
                "   - NEVER-ENGAGED feeds publishing actively -- suggest unsubscribe or try reading for a week",
                "   - HIGH-ENGAGEMENT feeds -- highlight as keepers",
                "4. Present a summary table with columns: Feed, Folder, Status, Engagement Rate, Articles, Recommendation.",
                "5. Summarize the category-level priors from either response (they are identical) to show which folders have the best engagement.",
                "6. Ask for confirmation before executing any unsubscribes via manage_subscription. For large prune sets, confirm the count explicitly before acting.",
              ].join("\n"),
            },
          },
        ],
